...
PY
  ...
  mkdir -p "$(dirname "${MUSUBI_ENV_FILE}")"
  {
    ...
    echo "export UV_LINK_MODE=\"\${UV_LINK_MODE:-copy}\""
...
run_with_retries "torch stack pin" uv pip install --no-cache-dir --force-reinstall \
...
write_musubi_env
if ! verify_torch "after pin attempt"; then
  echo "[bootstrap] ERROR: torch still cannot be imported after repair."
  echo "[bootstrap] If this is a transient network problem, restart the pod after DNS recovers."
  echo "[bootstrap] If it repeats, try MUSUBI_CUDA_EXTRA=cu126 with matching torch pins."
  exit 1
fi
...
run_with_retries "bitsandbytes install" uv pip install --no-cache-dir bitsandbytes || \
...
run_with_retries "hf_transfer install" uv pip install --no-cache-dir hf_transfer || \
...
run_with_retries "prodigyopt install" uv pip install --no-cache-dir prodigyopt || \
